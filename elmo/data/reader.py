...
class BaseReader(object):
    ...
    def file_based_convert_examples_to_features(self, input_file, vocab_file, output_file):
        nlp_schema = {
            "tokens_characters":{"type": "int32", "shape":[self.num_steps, self.max_word_length]},
            "tokens_characters_reverse":{"type": "int32", "shape":[self.num_steps, self.max_word_length]},
...
def create_elmo_dataset(batch_size=1, repeat_count=1, data_file_path=None,
    ...
    type_cast_op_float = C.TypeCast(mstype.int32)
    data_set = data_set.map(operations=type_cast_op_float, input_columns=columns_list[0])
    data_set = data_set.map(operations=type_cast_op_float, input_columns=columns_list[1])
    data_set = data_set.map(operations=type_cast_op_float, input_columns=columns_list[2])
    data_set = data_set.map(operations=type_cast_op_float, input_columns=columns_list[3])
